...
[Stage B readiness/gap assessment](../reviews/ticket-19/stage-b-readiness.md): current `claude` CLI 2.1.272 self-documentation captured; the official `claude-api` skill is now installed; and the P1 local routing probe (prototype commit `184f163`) measured that the client honors a base-URL override with a per-Run sentinel over streaming SSE. Remaining pre-Stage-B work: the TLS fifth-endpoint subset, billing/rates preflight, and the experiment card. Stage B remains NOT RUN and separately unauthorized; no blocker edge changes.
