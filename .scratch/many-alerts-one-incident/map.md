...
- One Kubernetes-native Fault is in the story, so Kubernetes is real, gated by the capacity prototype. **The capacity gate passed and the signal gate did not**: ticket 26 found the nominated flag emits no Kubernetes signal at all, so which Fault is the Kubernetes-native one is reopened and belongs to ticket 10. The fallback is compose with Kubernetes resource attributes and synthesized Kubernetes Events, decided on the map rather than in rehearsal.
