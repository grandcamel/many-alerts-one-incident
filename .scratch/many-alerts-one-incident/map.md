...
- How the system is driven. The core layer drops the load generator, so a Fault produces nothing until something makes requests; ticket 08's `traffic.sh` is a stopgap. Whether the demo restores a trimmed load generator, scripts a presenter-driven flow, or something else. Waits on Faults.
