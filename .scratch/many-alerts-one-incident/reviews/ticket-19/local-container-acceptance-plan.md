...
Latest result: the user explicitly authorized [C1 attempt 2](c1-attempt-2-outcome.md). The PEM correction passed the full source suite (365 passed / 36 skipped); live provisioning, baseline and identity/filesystem checks then passed. The attempt stopped on additional Run tunnel-interface names despite an empty IPv4 route table, before admission or reachability/native measurements. Cleanup is verified; the network claim remains inconclusive pending interface-state evidence and a reviewed predicate. No third attempt follows automatically.
...
| Line count versus byte cap | Samples below/at/above the configured line count and 10 MiB backend-body boundary; explicit overflow error, no silently shortened success. Use small per-case bundles that fit the capture quota. | Controlled synthetic backend, C1 |
