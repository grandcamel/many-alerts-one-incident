...
4. Assemble an individual-file Run allowlist: only the three `run/` texts, pinned Notification
   ...
   operator material outside its namespace. Exclude parent README/manifests, Ground truth,
...
6. Establish ticket 36's mediated native path, exact model/effort availability and identity,
   effective permissions, routing/TLS/streaming/revocation/direct-route prevention. None is
   supplied by these texts. Missing support closes the gate; no direct-credential fallback.
7. Verify the 270/20/10 supervisor and full durable-closeout bound, private sanitized audit
